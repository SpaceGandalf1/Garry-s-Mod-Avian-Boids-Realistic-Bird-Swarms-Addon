if SERVER then
    concommand.Add("boids_reset_server", function(ply, cmd, args)
        if IsValid(ply) and not ply:IsSuperAdmin() then
            ply:ChatPrint("You must be a superadmin to reset server-wide boid settings.")
            return
        end
...
if CLIENT then
    concommand.Add("boids_reset_client", function(ply, cmd, args)
        ...
        ply:ChatPrint("All client-side boid and fish settings have been reset to defaults.")
    end)
end