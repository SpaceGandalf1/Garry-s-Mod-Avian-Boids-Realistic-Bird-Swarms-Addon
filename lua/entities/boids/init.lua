...
function ENT:CustomThink()
    ...
    if self.LandingTarget then
        ...
        elseif pos:DistToSqr(self.LandingTarget) < (45 * 45) then
            ...
            -- We don't snap position directly to prevent floating issues. Instead, set position explicitly.
            local snapTr = util.TraceLine({
                start = pos, endpos = pos - Vector(0,0,50), mask = MASK_SOLID, filter = self
            })
            if snapTr.Hit then
                -- Need to add a slight z offset to prevent boids getting stuck in ground or going completely invisible.
                self:SetPos(snapTr.HitPos + Vector(0,0,5))
            else
                self:SetPos(pos)
            end
            ...
            -- Reset network origin too so client side lerp works
            self:SetNetworkOrigin(self:GetPos())
